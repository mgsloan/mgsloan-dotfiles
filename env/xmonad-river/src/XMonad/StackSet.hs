...
-- Vendored verbatim from xmonad's src/XMonad/StackSet.hs.
--
-- This module is entirely pure -- it has no X11 import and never did -- so the
-- window type parameter is instantiated to a river protocol object instead of
-- an X11 resource id and nothing else changes. Keeping it byte-identical to
-- upstream means xmonad's QuickCheck properties still apply to it unmodified.
module XMonad.StackSet (
        -- * Introduction
        -- $intro
